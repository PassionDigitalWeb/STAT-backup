...
export default class Sync {
    ...
    async retrieveKeywords() {
        ...
        while (!finished) {
            const res = await getSiteDataSTAT(
                this.site.Id,
                startRow,
                Sync.ROW_LIMIT
            )
    ...
    async syncKeywords() {
        ...
        const keywordInserts: KeywordInsert[] = keywords.map((keyword) => ({
            ...
            GlobalSearchVolume:
                parseInt(keyword?.KeywordStats?.GlobalSearchVolume) || 0,
            RegionalSearchVolume:
                parseInt(keyword?.KeywordStats?.RegionalSearchVolume) || 0,
        ...
        for (const keyword of this.keywords) {
            if (keyword?.KeywordRanking?.date) {
                const rankingInsert: KeywordRankingInsert = {
                    ...
                    BaseRank:
                        parseInt(keyword?.KeywordRanking?.Google?.BaseRank) ||
                        0,
